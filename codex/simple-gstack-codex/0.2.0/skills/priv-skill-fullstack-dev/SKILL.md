...
- Escalate to `$gstack-plan` or `$priv-skill-eng-planning` when the change alters architecture, auth, persistence contracts, migrations, release topology, or more than one vertical slice.
- Escalate to `$priv-skill-bug-fix` when the request is primarily a defect with unknown cause.
