...
Use `$gstack-qa` for multi-finding QA repair, `$gstack-investigate` if added later for deep incidents, or `$priv-skill-eng-planning` when the fix requires architectural change.
