...
3. Classify each candidate:
   ...
   - `NEW`: deserves a new `skill-*` or `gstack-*` skill
...
4. Map candidates to local targets:
   ...
   - update existing `skill-*` for lightweight point capability
   - add a new `skill-*` only when a user-recognizable trigger and reusable workflow exist
